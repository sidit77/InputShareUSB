...
impl InputEvent {
    ...
    pub fn to_key_event(&self) -> Option<KeyEvent> {
        match self {
            InputEvent::KeyboardKeyEvent(key, _, state) => Some(KeyEvent { key: *key, state: *state }),
            InputEvent::MouseButtonEvent(key, state) => Some(KeyEvent { key: *key, state: *state }),
            _ => None
        }
    }
}
...
impl VirtualKey {
    pub fn is_mouse_button(self) -> bool {
        matches!(
            self,
            VirtualKey::LButton | VirtualKey::MButton | VirtualKey::RButton | VirtualKey::XButton1 | VirtualKey::XButton2
        )
    }
}
        ...
        match KEY_NAMES[u8::from(*self) as usize] {
            ...
            Some(name) => match name.contains("OEM") {
                true => {
                    ...
                    let iter = char::decode_utf16(buffer[..length].iter().copied()).map(|r| r.unwrap_or(REPLACEMENT_CHARACTER));
                    let mut start = true;
                    for mut c in iter {
                        if c.is_whitespace() {
                            start = true;
