...
use crate::{Input, KeyState, ScrollDirection, VirtualKey, WinResult};
...
fn add_to_vec(vec: &mut Vec<INPUT>, input: Input) {
    match input {
        ...
        Input::MouseButtonInput(key, state) => vec.push(create_mouse_input(MOUSEINPUT {
            dx: 0,
            dy: 0,
            mouseData: match key {
            ...
            dwFlags: match (key, state) {
                ...
                _ => {
                    tracing::warn!("Unsupported key ({:?}): Skipping!", key);
                    MOUSEEVENTF_WHEEL
                }
            },
            time: 0,
        ...
        Input::RelativeMouseMoveInput(dx, dy) => vec.push(create_mouse_input(MOUSEINPUT {
            dx,
            dy,
            mouseData: 0,
        ...
        Input::AbsoluteMouseMoveInput(x, y) => vec.push(create_mouse_input(MOUSEINPUT {
...
pub fn send_inputs<'a>(inputs: impl IntoIterator<Item = Input<'a>>) -> WinResult<()> {
    ...
    let c = unsafe { SendInput(&vec, size_of::<INPUT>() as i32) };
